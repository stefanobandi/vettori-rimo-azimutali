...
<style>
    ...
    @media (max-width: 640px) { [data-testid="stMetricValue"] { font-size: 1.2rem !important; } }
</style>
...
st.markdown(f"<div style='text-align: center;'><p style='font-size: 14px;'>Bollard Pull: 70 ton | Logica: Intersezione-Centro Ponderato</p></div>", unsafe_allow_html=True)
st.write("---")
...
with st.sidebar:
    ...
    show_prediction = st.checkbox("Predizione 30s", value=True)
    st.markdown("---")
...
M_tm = ((pos_sx-pp_pos)[0]*F_sx_eff[1] - (pos_sx-pp_pos)[1]*F_sx_eff[0] + (pos_dx-pp_pos)[0]*F_dx_eff[1] - (pos_dx-pp_pos)[1]*F_dx_eff[0])
...
with col_c:
    ...
    if res_ton > 0.1:
        ...
        ax.arrow(origin_res[0], origin_res[1], res_u*sc, res_v*sc, fc='blue', ec='blue', width=0.3, head_width=min(0.8, v_res_len*0.4), head_length=min(1.2, v_res_len*0.5), alpha=0.8, zorder=6, length_includes_head=True)

    # 7. VETTORI PROPULSIONE (zorder 7)
    v_sx_len, v_dx_len = np.linalg.norm(F_sx_eff)*sc, np.linalg.norm(F_dx_eff)*sc
    ax.arrow(pos_sx[0], pos_sx[1], F_sx_eff[0]*sc, F_sx_eff[1]*sc, fc='red', ec='red', width=0.15, head_width=min(0.5, v_sx_len*0.4), head_length=min(0.7, v_sx_len*0.5), zorder=7, alpha=0.8, length_includes_head=True)
    ax.arrow(pos_dx[0], pos_dx[1], F_dx_eff[0]*sc, F_dx_eff[1]*sc, fc='green', ec='green', width=0.15, head_width=min(0.5, v_dx_len*0.4), head_length=min(0.7, v_dx_len*0.5), zorder=7, alpha=0.8, length_includes_head=True)

    # 8. ICONE ELICHE E PIVOT (zorder 8-10)
    ...
    m4.metric("Momento", f"{abs(M_tm):.0f} tm")
