...
# --- SOLUTORE FAST SIDE STEP (LOGICA RICHIESTA) ---
def apply_fast_side_step(direction):
    ...
    try:
        if direction == "DRITTA":
            # Motore SX (Drive): Fisso 045° / 50%
            a_drive, p_drive = 45.0, 50.0
            x_drive, x_slave = -POS_THRUSTERS_X, POS_THRUSTERS_X
            
            # 1. Punto di intersezione sulla coordinata Y del Pivot Point
            x_int = x_drive + dist_y * np.tan(np.radians(a_drive))
            
            # 2. Angolo del motore DX (Slave) per puntare all'intersezione
            dx = x_slave - x_int
            dy = POS_THRUSTERS_Y - pp_y
            a_slave = np.degrees(np.arctan2(dx, dy)) % 360
            
            # 3. Calcolo potenza per equilibrio longitudinale (risultante 090°)
            cos_drive = np.cos(np.radians(a_drive))
            cos_slave = np.cos(np.radians(a_slave))
            
            if abs(cos_slave) < 0.001: raise ValueError("Angolo Slave critico (divisione per zero)")
            ...
            if 0 <= p_slave <= 100:
                ...
                st.toast(f"Fast Dritta impostato: Slave {int(round(p_slave))}%", icon="⚡")
            else:
                st.error(f"Nessuna soluzione: Potenza DX necessaria ({p_slave:.1f}%) fuori range 0-100.")

        else: # SINISTRA
            # Motore DX (Drive): Fisso 315° / 50%
            ...
            if 0 <= p_slave <= 100:
                ...
                st.toast(f"Fast Sinistra impostato: Slave {int(round(p_slave))}%", icon="⚡")
            else:
                st.error(f"Nessuna soluzione: Potenza SX necessaria ({p_slave:.1f}%) fuori range 0-100.")
    except Exception as e:
        st.error(f"Errore matematico: {e}")
...
# Calcolo geometrico intersezione per tratteggio
...
with st.sidebar:
    ...
    if st.button("Reset Motori & Pivot", type="primary", use_container_width=True):
        reset_engines(); reset_pivot()
    st.markdown("---")
    st.markdown("### ↔️ Fast Side Step")
    c1, c2 = st.columns(2)
...
with col_center:
    st.slider("Pivot Point Y (Longitudinale)", -16.0, 16.0, step=0.1, key="pp_y")
    ...
    # Pivot Point
    ax.scatter(st.session_state.pp_x, st.session_state.pp_y, c='black', s=150, zorder=10, label="PP")
    ax.text(st.session_state.pp_x+0.8, st.session_state.pp_y, "PIVOT", fontweight='bold')

    # Tratteggio intersezione (Blu fine)
    if inter_pt is not None and abs(inter_pt[1]) < 60:
        ax.plot([pos_sx[0], inter_pt[0]], [pos_sx[1], inter_pt[1]], color='red', linestyle='--', lw=0.7, alpha=0.5)
        ax.plot([pos_dx[0], inter_pt[0]], [pos_dx[1], inter_pt[1]], color='green', linestyle='--', lw=0.7, alpha=0.5)
    ...
    m3.metric("Momento", f"{Total_Moment:.1f} tm", delta="Rotazione" if abs(Total_Moment) > 1 else "Stabile")
